...
if [ "$2" ]; then
    if test -f $2; then
        # for the dump file:
        echo "Writing the sampled intervals to a dump file"
        ...
        count=0
        rm sampled_every_$1_better.dump
        for i in {$first_timestep..$last_timestep..$period}; do
            if [ $(($i % $1)) -eq 0 ]; then
                First_timestep=$i
                First_occurance=$(($first_occurance + $period_line * $count))
                break
            fi
            count=$(($count + 1))
        done
        count=0
        for i in {$First_timestep..$last_timestep..$1}; do
            if [ $(($i % $1)) -eq 0 ]; then
                tail -n +$(($First_occurance + $period_line * $count * $1 / $period)) $2 | head -n $period_line >>sampled_every_$1_better.dump
            fi
            count=$(($count + 1))
        done
...
if [ "$3" ]; then
    if test -f $3; then
        # for the bonds file:
        echo "Writing the sampled intervals to a bonds file"
        ...
        number_of_timestep=$((1 + $difference / $period))
        count=0
        rm sampled_every_$1_better.bonds
        for i in {$(($first_timestep))..$(($last_timestep))..$(($period))}; do
            if [ $(($i % $1)) -eq 0 ]; then
                First_timestep=$i
                First_occurance=$(($first_occurance + $period_line * $count))
        ...
        for i in {$(($First_timestep))..$(($last_timestep))..$(($1))}; do
            if [ "$(($i % $1))" -eq 0 ]; then
                tail -n +$(($First_occurance + $period_line * $count * $1 / $period)) $3 | head -n $period_line >>sampled_every_$1_better.bonds
            fi
            count=$(($count + 1))
        done
        echo "Done"
